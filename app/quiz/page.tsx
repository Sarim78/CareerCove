...
/** Industry filter options shown on the results screen. */
...
/** All eight quiz questions with their radio options. */
const QUESTIONS = [
  {
    id: 1,
  ...
  },
  {
    ...
    options: [
      ...
      { value: "client-facing",       label: "Dynamic environments with regular client interaction and relationship building" },
    ],
  },
  {
    id: 5,
  ...
  },
  {
    ...
    options: [
      ...
      { value: "self-directed-independent", label: "Self-paced learning through books, courses, and personal exploration" },
    ],
  },
  {
    id: 7,
...
/**
 * Career recommendation lookup table keyed by hyphen-joined answer values.
 * Falls back to `default` when no exact or partial match is found.
 */
const CAREER_RECOMMENDATIONS: Record<string, {
  title: string; description: string; skills: string[]; growth: string;
  salary: string; relatedRoles: string[]; fields: string[]
}> = {
  "analytical-deep-global-scale-analytical-data-research-focused-technical-innovation-theoretical-research-innovation-cutting-edge-systematic-planning": {
    ...
    description: "Lead advanced analytics projects using machine learning and AI to solve complex business problems at scale.",
  ...
  "systematic-structured-organizational-strategic-interactive-verbal-collaborative-open-strategic-business-mentorship-social-growth-development-team-support": {
    ...
    description: "Drive product strategy and development, working cross-functionally to deliver innovative solutions that meet market needs.",
  ...
  "creative-innovative-individual-personal-visual-design-collaborative-open-creative-expression-hands-on-practical-innovation-cutting-edge-adaptive-flexible": {
    ...
    description: "Create intuitive, user-centered designs that solve complex problems and enhance user experiences across digital platforms.",
  ...
  "analytical-deep-global-scale-analytical-data-fast-paced-dynamic-technical-innovation-hands-on-practical-innovation-cutting-edge-independent-focused": {
    ...
    description: "Design and build scalable software systems, working with cutting-edge technologies to create robust applications.",
  ...
  "systematic-structured-organizational-strategic-narrative-written-client-facing-strategic-business-mentorship-social-influence-leadership-team-support": {
    ...
    description: "Advise organizations on strategic initiatives, operational improvements, and transformation projects.",
    skills: ["Strategic Analysis", "Project Management", "Client Relations", "Business Process Improvement", "Leadership"],
  ...
  "creative-innovative-global-scale-visual-design-collaborative-open-creative-expression-mentorship-social-growth-development-adaptive-flexible": {
    ...
    description: "Develop and execute comprehensive digital marketing strategies to build brand awareness and drive customer engagement.",
  ...
  "collaborative-social-individual-personal-interactive-verbal-collaborative-open-human-psychology-mentorship-social-growth-development-team-support": {
    ...
    description: "Provide therapeutic services and psychological assessments to help individuals overcome mental health challenges.",
  ...
  "analytical-deep-organizational-strategic-analytical-data-research-focused-strategic-business-theoretical-research-stability-security-systematic-planning": {
    ...
    description: "Analyze financial markets, evaluate investment opportunities, and provide recommendations for portfolio management.",
  ...
  "collaborative-social-individual-personal-narrative-written-collaborative-open-human-psychology-mentorship-social-growth-development-team-support": {
    ...
    skills: ["Curriculum Design", "Adult Learning Theory", "Training Delivery", "Performance Analysis", "Leadership Development"],
    growth: "Growing importance in corporate learning",
    salary: "$65,000 - $110,000",
    relatedRoles: ["Training Specialist", "Instructional Designer", "HR Business Partner", "Organizational Development Consultant"],
  ...
  "analytical-deep-global-scale-narrative-written-research-focused-technical-innovation-theoretical-research-stability-security-independent-focused": {
    ...
    description: "Conduct advanced research in your field of expertise, publish findings, and contribute to scientific knowledge.",
    skills: ["Research Methodology", "Statistical Analysis", "Scientific Writing", "Grant Writing", "Critical Thinking"],
  ...
  default: {
    ...
    description: "Transform data into actionable insights that drive strategic business decisions and operational improvements.",
...
/**
 * Picks the best career recommendation given the user's answers.
 * Tries exact key match first, then pattern-based partial matching, then default.
 */
function getRecommendation(answers: Record<number, string>) {
  const values  = Object.values(answers)
  const exactKey = values.join("-")

  if (CAREER_RECOMMENDATIONS[exactKey]) return CAREER_RECOMMENDATIONS[exactKey]

  // Partial pattern matching for common answer combinations
  if (values.includes("analytical-deep") && values.includes("technical-innovation"))
    return CAREER_RECOMMENDATIONS["analytical-deep-global-scale-analytical-data-research-focused-technical-innovation-theoretical-research-innovation-cutting-edge-systematic-planning"]
  if (values.includes("creative-innovative") && values.includes("visual-design"))
    return CAREER_RECOMMENDATIONS["creative-innovative-individual-personal-visual-design-collaborative-open-creative-expression-hands-on-practical-innovation-cutting-edge-adaptive-flexible"]
  if (values.includes("strategic-business") && values.includes("influence-leadership"))
    return CAREER_RECOMMENDATIONS["systematic-structured-organizational-strategic-interactive-verbal-collaborative-open-strategic-business-mentorship-social-growth-development-team-support"]
  if (values.includes("human-psychology") && values.includes("individual-personal"))
    return CAREER_RECOMMENDATIONS["collaborative-social-individual-personal-interactive-verbal-collaborative-open-human-psychology-mentorship-social-growth-development-team-support"]

  return CAREER_RECOMMENDATIONS.default
}

// ── Component ─────────────────────────────────────────────────────────────────

/**
 * QuizPage — multi-step career assessment quiz.
 *
 * Users answer 8 questions; answers are matched against CAREER_RECOMMENDATIONS
 * to produce a personalised career suggestion with skills, salary, and related roles.
 * Results can be filtered by industry on the results screen.
 */
export default function QuizPage() {
  ...
  /** Advances or completes the quiz with a brief fade animation. */
  ...
  const toggleFilter   = (id: string) => setSelectedFilters((prev) => prev.includes(id) ? prev.filter((f) => f !== id) : [...prev, id])
  const resetQuiz      = () => { setCurrentQuestion(0); setAnswers({}); setShowResults(false); setSelectedFilters([]) }

  // ── Results screen ───────────────────────────────────────────────────────────

  if (showResults) {
    const recommendation = getRecommendation(answers)
    const isVisible      = selectedFilters.length === 0 || recommendation.fields.some((f) => selectedFilters.includes(f))
        ...
        <div className="container mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-8 sm:py-12 md:py-16">
          <div className="max-w-3xl mx-auto">
            <div className="text-center mb-12 animate-fade-in">
              ...
              {!showFilters ? (
                <div className="mb-8">
                  <Button onClick={() => setShowFilters(true)} variant="outline" className="border-primary-300 text-primary-700 hover:bg-primary-50">
                    Filter by Industry
                  </Button>
                </div>
              ) : (
                <Card className="mb-8 bg-white border-primary-200/50 shadow-lg">
                  <CardHeader className="pb-4">
                    <div className="flex items-center justify-between">
                      ...
                      <Button onClick={() => setShowFilters(false)} variant="ghost" size="sm" className="text-primary-600 hover:text-primary-800">
                        <X className="w-4 h-4" />
                      </Button>
                    </div>
                    <CardDescription className="text-primary-600">Select industries to see relevant career matches</CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="flex flex-wrap gap-3">
                      {FILTER_CATEGORIES.map((cat) => (
                        <Button key={cat.id} onClick={() => toggleFilter(cat.id)} size="sm"
                          variant={selectedFilters.includes(cat.id) ? "default" : "outline"}
                          className={`transition-all duration-300 ${selectedFilters.includes(cat.id) ? "bg-primary-500 hover:bg-primary-600 text-white" : "border-primary-300 text-primary-700 hover:bg-primary-50"}`}
                        >
                          {cat.label}
                        </Button>
                      ))}
                    ...
                    {selectedFilters.length > 0 && (
                      <div className="flex items-center justify-between mt-4 pt-4 border-t border-primary-200">
                        <span className="text-sm text-primary-600">{selectedFilters.length} filter{selectedFilters.length !== 1 ? "s" : ""} selected</span>
                        <Button onClick={() => setSelectedFilters([])} variant="ghost" size="sm" className="text-primary-600 hover:text-primary-800">Clear all</Button>
                      </div>
                    )}
                  </CardContent>
                </Card>
              ...
              <p className="text-primary-600 text-lg">Based on your responses, here's your personalised career recommendation</p>
            </div>
            ...
            {isVisible ? (
              <Card className="hover-lift bg-white border-primary-200/50 shadow-xl animate-scale-in">
                <CardHeader className="text-center pb-6">
                  ...
                  <CardDescription className="text-lg text-primary-600 leading-relaxed">{recommendation.description}</CardDescription>
                </CardHeader>
                <CardContent className="space-y-8">
                  <div className="grid gap-6 md:grid-cols-2">
                    <div className="p-6 rounded-xl bg-primary-50 border border-primary-200">
                      <h3 className="font-semibold text-primary-800 mb-4 flex items-center">
                        <TrendingUp className="w-5 h-5 mr-2" /> Key Skills to Develop
                      </h3>
                      <div className="flex flex-wrap gap-2">
                        {recommendation.skills.map((skill) => (
                          <span key={skill} className="px-3 py-2 bg-white text-primary-700 rounded-lg text-sm font-medium shadow-sm">{skill}</span>
                        ))}
                      </div>
                    </div>
                  ...
                  <div className="p-6 rounded-xl bg-blue-50 border border-blue-200">
                    ...
                    <div className="grid gap-2 md:grid-cols-2">
                      {recommendation.relatedRoles.map((role) => (
                        <div key={role} className="flex items-center text-sm text-blue-700">
                          <span className="w-2 h-2 bg-blue-400 rounded-full mr-3" /> {role}
                        </div>
                      ))}
                    </div>
                  </div>
                  <div className="flex flex-col sm:flex-row gap-4">
                    <Button asChild className="flex-1 bg-primary-500 hover:bg-primary-600 transition-all duration-300 hover:scale-105">
                      <Link href="/mentors">Find a Mentor</Link>
                    </Button>
                    <Button asChild variant="outline" className="flex-1 border-primary-300 text-primary-700 hover:bg-primary-50 transition-all duration-300 hover:scale-105 bg-transparent">
                      <Link href="/resume">Enhance Resume</Link>
                    </Button>
                  </div>
                  <Button variant="ghost" onClick={resetQuiz} className="w-full text-primary-600 hover:text-primary-800 hover:bg-primary-50 transition-all duration-300">
                    Retake Quiz
                  </Button>
                </CardContent>
              </Card>
              ...
              <Card className="bg-white border-primary-200/50 shadow-lg">
                <CardContent className="text-center py-12">
                  ...
                  <p className="text-primary-600 mb-6">Your career recommendation doesn't match the selected industries. Try adjusting or clearing your filters.</p>
                  <Button onClick={() => setSelectedFilters([])} className="bg-primary-500 hover:bg-primary-600 transition-all duration-300">Clear Filters</Button>
                </CardContent>
              </Card>
            )}
          </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-white via-primary-50 to-primary-100">
      ...
      <div className="container mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-8 sm:py-12 md:py-16">
        <div className="max-w-3xl mx-auto">
          ...
          {/* Question card */}
          <Card className={`hover-lift bg-white border-primary-200/50 shadow-xl transition-all duration-300 ${isAnimating ? "opacity-50 scale-95" : "opacity-100 scale-100"}`}>
            <CardHeader className="pb-4 sm:pb-6 px-3 sm:px-4 md:px-6">
              <CardTitle className="text-lg sm:text-xl md:text-2xl text-primary-800 leading-relaxed">{question.question}</CardTitle>
              <CardDescription className="text-xs sm:text-sm text-primary-600 mt-2">Select the option that best describes your preferences</CardDescription>
            </CardHeader>
            <CardContent className="px-3 sm:px-4 md:px-6">
              <RadioGroup value={currentAnswer} onValueChange={handleAnswer} className="space-y-2 sm:space-y-3 md:space-y-4">
                {question.options.map((option) => (
                  <div key={option.value} className="flex items-start space-x-2 sm:space-x-3 p-2 sm:p-3 md:p-4 rounded-lg sm:rounded-xl border border-primary-200 hover:border-primary-300 hover:bg-primary-50 transition-all duration-300 cursor-pointer group">
                    <RadioGroupItem value={option.value} id={option.value} className="mt-0.5 sm:mt-1 flex-shrink-0" />
                    <Label htmlFor={option.value} className="flex-1 cursor-pointer text-xs sm:text-sm md:text-base text-primary-700 group-hover:text-primary-800 transition-colors duration-300">
                      {option.label}
                    </Label>
                  </div>
                ))}
              ...
              <div className="flex flex-col xs:flex-row gap-2 sm:gap-3 mt-6 sm:mt-8 md:mt-10">
                <Button variant="outline" onClick={prevQuestion} disabled={currentQuestion === 0}
                  ...
                  <ArrowLeft className="mr-1 sm:mr-2 h-3 sm:h-4 w-3 sm:w-4" /> Previous
                </Button>
                <Button onClick={nextQuestion} disabled={!currentAnswer || isAnimating}
