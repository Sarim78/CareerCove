...
export default function CareerPathPage() {
  ...
  const checkUser = async () => {
    ...
    if (session?.user) {
      loadCareerPaths(session.user.id)
    } else {
      setLoading(false)
    }
  }

  const loadCareerPaths = async (userId: string) => {
    try {
  ...
  const saveCareerPath = async () => {
    if (!user || !newPath.careerTitle || !newPath.currentRole || !newPath.targetRole) return
  ...
  const deleteCareerPath = async (pathId: string) => {
    ...
    try {
      const { error } = await supabase.from("career_paths").delete().eq("id", pathId)
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-primary-50 via-white to-accent-50">
      ...
      <main className="flex-1 px-4 sm:px-6 lg:px-8 py-6 sm:py-8 lg:py-12 max-w-7xl mx-auto w-full">
        ...
        {!user ? (
          <Card className="p-8 sm:p-12 text-center border-primary-100 mb-8">
            <TrendingUp className="w-16 h-16 mx-auto text-primary-300 mb-4" />
            <h3 className="text-lg sm:text-xl font-semibold text-primary-900 mb-2">Sign in to plan your career path</h3>
            <p className="text-sm sm:text-base text-primary-600 mb-4">
              Create an account to build and visualize your personalized career roadmap
            </p>
            <Button
                        ...
                        <div key={index} className="flex items-start gap-2 p-3 bg-primary-50 rounded-lg">
                          <div className="flex-1">
                            ...
                            {milestone.description && (
                              <p className="text-xs text-primary-600">{milestone.description}</p>
                            )}
                          </div>
                          <Button
                            variant="ghost"
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        <div>
                          <Input
                            ...
                            onChange={(e) =>
                              setNewMilestone({ ...newMilestone, year: Number.parseInt(e.target.value) })
                            }
                            className="text-sm"
                          />
                        </div>
                  ...
                  <Card key={path.id} className="border-primary-200 overflow-hidden">
                    <CardHeader className="bg-gradient-to-r from-primary-50 to-accent-50 border-b border-primary-100">
                      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                        <div className="flex-1">
                          <CardTitle className="text-xl sm:text-2xl text-primary-900 mb-2">
                            {path.career_title}
                          </CardTitle>
