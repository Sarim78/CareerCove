...
const INITIAL_PATHS: CareerPath[] = [
  {
    id: "1",
...
const EMPTY_PATH = { careerTitle: "", currentRole: "", targetRole: "", timelineYears: 5, milestones: [] as Milestone[] }
const EMPTY_MILESTONE = { year: 1, title: "", description: "" }

/**
 * CareerPathPage — a drag-free career roadmap builder.
 *
 * Users can create named career paths with a current → target role, a
 * multi-year timeline, and ordered milestones. All state is local (no backend).
 */
export default function CareerPathPage() {
  const [careerPaths, setCareerPaths] = useState<CareerPath[]>(INITIAL_PATHS)
  ...
  const addMilestone = () => {
    ...
    setNewPath((p) => ({
      ...p,
      milestones: [...p.milestones, { ...newMilestone, completed: false }],
    }))
    setNewMilestone({ year: newPath.milestones.length + 2, title: "", description: "" })
  }

  const removeMilestone = (index: number) =>
    setNewPath((p) => ({ ...p, milestones: p.milestones.filter((_, i) => i !== index) }))
  ...
  const deletePath = (id: string) => setCareerPaths((prev) => prev.filter((p) => p.id !== id))
    ...
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-primary-50 via-white to-accent-50">
      ...
      <main className="flex-1 px-4 sm:px-6 lg:px-8 py-6 sm:py-8 lg:py-12 max-w-7xl mx-auto w-full">
        <div className="text-center mb-8 sm:mb-12">
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-primary-900 mb-3 sm:mb-4">Career Path Simulator</h1>
        ...
        <div className="mb-6">
          <Button onClick={() => setShowAdd(!showAdd)} className="w-full sm:w-auto bg-primary-500 hover:bg-primary-600 text-white">
            <Plus className="w-4 h-4 mr-2" /> Create Career Path
          </Button>
        </div>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <Input placeholder="e.g., Software Engineering" value={newPath.careerTitle}
                ...
                <div>
                  ...
                  <Input type="number" min="1" max="20" value={newPath.timelineYears}
                    onChange={(e) => setNewPath({ ...newPath, timelineYears: parseInt(e.target.value) })} />
                </div>
                <div>
                  <label className="text-sm font-medium text-primary-900 mb-1.5 block">Current Role</label>
                  <Input placeholder="e.g., Junior Developer" value={newPath.currentRole}
                ...
                <div>
                  ...
                  <Input placeholder="e.g., Senior Engineer" value={newPath.targetRole}
              ...
              <div className="border-t border-primary-200 pt-4">
                ...
                {newPath.milestones.map((m, i) => (
                  <div key={i} className="flex items-start gap-2 p-3 bg-primary-50 rounded-lg mb-2">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        <Badge variant="secondary" className="bg-primary-100 text-primary-700">Year {m.year}</Badge>
                        <span className="text-sm font-medium text-primary-900">{m.title}</span>
                      </div>
                      {m.description && <p className="text-xs text-primary-600">{m.description}</p>}
                    </div>
                    <Button variant="ghost" size="icon" onClick={() => removeMilestone(i)} className="hover:bg-red-50 hover:text-red-600 shrink-0">
                ...
                <div className="space-y-3 p-3 bg-primary-50/50 rounded-lg">
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <Input type="number" placeholder="Year" min="1" max={newPath.timelineYears} value={newMilestone.year}
                  ...
                  <Button onClick={addMilestone} variant="outline" size="sm" className="w-full border-primary-300 text-primary-700 hover:bg-primary-50 bg-transparent">
              ...
              <div className="flex gap-3 pt-4">
                <Button onClick={saveCareerPath} className="flex-1 bg-primary-500 hover:bg-primary-600 text-white">
                  <Target className="w-4 h-4 mr-2" /> Save Career Path
                </Button>
        ...
        {careerPaths.length === 0 ? (
          <Card className="p-8 sm:p-12 text-center border-primary-100">
            ...
            <p className="text-sm sm:text-base text-primary-600 mb-4">Create your first career path to visualize your professional journey</p>
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex flex-wrap gap-2 text-sm text-primary-700">
                        <Badge variant="outline" className="border-primary-300 bg-white">
                          <Calendar className="w-3 h-3 mr-1" /> {path.timeline_years} years
                        </Badge>
                        <span className="px-3 py-1">{path.current_role} → {path.target_role}</span>
                      </div>
                    </div>
                    <Button variant="ghost" size="icon" onClick={() => deletePath(path.id)} className="hover:bg-red-50 hover:text-red-600 self-start sm:self-center shrink-0">
                      <Trash2 className="w-4 h-4" />
                    </Button>
                  </div>
                </CardHeader>
                  ...
                  {path.milestones.length > 0 ? (
                    <div className="space-y-4">
                      <h4 className="text-sm font-semibold text-primary-900 flex items-center gap-2">
                        <Award className="w-4 h-4" /> Career Milestones
                      </h4>
                      <div className="relative pl-6 space-y-6">
                        <div className="absolute left-2 top-0 bottom-0 w-0.5 bg-primary-200" />
                        {path.milestones.map((m, i) => (
                          <div key={i} className="relative">
                            <div className="absolute left-[-1.6rem] top-1 w-4 h-4 rounded-full bg-primary-500 border-2 border-white shadow" />
                            <Card className="border-primary-100 bg-white">
                              <CardContent className="p-4">
                                <div className="flex items-start justify-between gap-2 mb-2">
                                  <Badge className="bg-primary-100 text-primary-700 shrink-0">Year {m.year}</Badge>
                                  {m.completed && <CheckCircle2 className="w-5 h-5 text-green-600 shrink-0" />}
                                </div>
                                <h5 className="font-semibold text-primary-900 mb-1">{m.title}</h5>
                                {m.description && <p className="text-sm text-primary-600">{m.description}</p>}
                              </CardContent>
                            </Card>
                          </div>
                        ))}
